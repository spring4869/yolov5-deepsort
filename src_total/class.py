...
PATH_DATASET = 'D:\Program Files\python\yolo\crops'  # 需要处理的文件夹
PATH_NEW_DATASET = 'D:\Program Files\python\yolo\stitches'  # 处理后的文件夹
        ...
        for aroot, dirs, files in os.walk(self.path):
            ...
            for item in filelist:
                if item.endswith('.JPEG'):  # 初始的图片的格式为jpg格式的（或者源文件是png格式及其他格式，后面的转换格式就可以调整为自己需要的格式即可）
                    ...
                    # 修改图片尺寸到128宽*256高
                    im = Image.open(src)
                    out = im.resize((256, 128), Image.ANTIALIAS)  # resize image with high-quality
                    out.save(src)  # 原路径保存
            ...
            for item in filelist:
                if item.endswith('.JPEG'):  # 初始的图片的格式为jpg格式的（或者源文件是png格式及其他格式，后面的转换格式就可以调整为自己需要的格式即可）
                    ...
                    dst = os.path.join(os.path.abspath(new_dir),
                                       dirname + 'C1T0001F' + str(num_pic + 1) + '.JPEG')
